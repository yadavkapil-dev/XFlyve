...
module.exports = {
  "/api/worklogs/admin": {
    get: {
      ...
      security: h.bearer,
      parameters: [h.pageParam, h.limitParam, h.sortParam(["workDate", "date", "createdAt"], "workDate"), { name: "status", in: "query", schema: { type: "string", enum: ["pending", "approved", "rejected"] } }, h.dateFromParam("workDate"), h.dateToParam("workDate")],
      responses: {
        200: { description: "OK.", content: { "application/json": { example: { success: true, data: [{ _id: FAKE_LOG_ID, status: "pending" }], pagination: { page: 1, limit: 20, total: 1, totalPages: 1 } } } } },
        401: h.unauthorized,
  ...
  "/api/worklogs/admin/pending": {
    get: {
      tags: ["Work Logs"],
      summary: "List pending work log approvals (admin only)",
      security: h.bearer,
      parameters: [h.pageParam, h.limitParam, h.sortParam(["workDate", "date", "createdAt"], "workDate"), { name: "driverId", in: "query", schema: { type: "string" } }, h.dateFromParam("workDate"), h.dateToParam("workDate")],
      responses: {
        200: { description: "OK.", content: { "application/json": { example: { success: true, data: [{ _id: FAKE_LOG_ID, status: "pending" }], pagination: { page: 1, limit: 20, total: 1, totalPages: 1 } } } } },
        401: h.unauthorized,
        403: h.forbiddenSuccessEnvelope("Access denied: Admins only"),
  ...
  "/api/worklogs/admin/{logId}/approve": {
    put: {
      tags: ["Work Logs"],
      summary: "Approve a work log (admin only)",
      security: h.bearer,
      parameters: [{ name: "logId", in: "path", required: true, schema: { type: "string" }, example: FAKE_LOG_ID }],
      responses: {
        200: { description: "Approved.", content: { "application/json": { example: { success: true, message: "Daily record approved", data: { _id: FAKE_LOG_ID, status: "approved" } } } } },
  ...
  "/api/worklogs/admin/{logId}/reject": {
    put: {
      tags: ["Work Logs"],
      summary: "Reject a work log with a reason (admin only)",
      security: h.bearer,
      parameters: [{ name: "logId", in: "path", required: true, schema: { type: "string" }, example: FAKE_LOG_ID }],
      requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["rejectionReason"], properties: { rejectionReason: { type: "string", example: "Hours don't match the job's scheduled window" } } } } } },
      responses: {
        200: { description: "Rejected.", content: { "application/json": { example: { success: true, message: "Daily record rejected", data: { _id: FAKE_LOG_ID, status: "rejected" } } } } },
        400: h.badRequestSuccess("Invalid logId"),
        401: h.unauthorized,
        403: h.forbiddenSuccessEnvelope("Access denied: Admins only"),
        404: h.notFoundSuccess("Work log"),
  ...
  "/api/worklogs": {
    post: {
      ...
      responses: {
        201: { description: "Created. Notifies admins for review.", content: { "application/json": { example: { success: true, message: "Work log created", data: { _id: FAKE_LOG_ID, status: "pending" } } } } },
  ...
  "/api/worklogs/{logId}": {
    put: {
      ...
      description: "Locked once approved (409). Editing a rejected log as the driver resubmits it (back to pending).",
      security: h.bearer,
      parameters: [{ name: "logId", in: "path", required: true, schema: { type: "string" }, example: FAKE_LOG_ID }],
      requestBody: { content: { "application/json": { schema: workLogBody } } },
      responses: {
        200: { description: "Updated.", content: { "application/json": { example: { success: true, message: "Work log updated", data: { _id: FAKE_LOG_ID, status: "pending" } } } } },
        ...
        403: h.forbiddenSuccessEnvelope("Access denied"),
        404: h.notFoundSuccess("Work log"),
        409: h.conflictSuccess("Approved records are locked and cannot be edited"),
        422: h.validationErrorSuccess("Hours must be a non-negative number"),
        500: h.serverErrorSuccess,
      },
    },
    delete: {
      ...
      description: "Blocked (409) once approved.",
      security: h.bearer,
      parameters: [{ name: "logId", in: "path", required: true, schema: { type: "string" }, example: FAKE_LOG_ID }],
      responses: {
        ...
        403: h.forbiddenSuccessEnvelope("Access denied"),
        404: h.notFoundSuccess("Work log"),
        409: h.conflictSuccess("Approved records are locked and cannot be deleted"),
        500: h.serverErrorSuccess,
      },
    },
  },
