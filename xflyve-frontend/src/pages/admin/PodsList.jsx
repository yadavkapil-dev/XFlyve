...
const AdminPODs = () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Server error loading pending POD approvals",
      );
    } finally {
  ...
  const handleDownload = async (pod) => {
    try {
      ...
      const dateStr = new Date(pod.uploadDate || Date.now())
        .toISOString()
        .slice(0, 10);
    ...
    try {
      ...
      setError(
        err.response?.status === 404
          ? "No approved POD files found for that date"
          : "Failed to download PODs",
      );
    } finally {
...
<Typography
  sx={{ mt: 1, color: alpha("#fff", 0.74), lineHeight: 1.6 }}
>
  Review proof-of-delivery uploads by driver for customer
  follow-up and invoice preparation.
</Typography>
<Stack
...
>
  ...
  <Button
    ...
    sx={{ minHeight: 40, borderRadius: 3, bgcolor: "white", color: palette.ink, fontWeight: 950, px: 2.5, whiteSpace: "nowrap", "&:hover": { bgcolor: alpha("#fff", 0.9) } }}
  >
    {downloadingAll ? "Preparing ZIP..." : "Download PODs"}
  </Button>
</Stack>
        </Paper>
            ...
            >
              <Box>
                <Typography
                  variant="h5"
                  fontWeight={950}
                  sx={{ color: palette.ink, letterSpacing: "-0.045em" }}
                >
                  Pending POD approvals
                </Typography>
                <Typography variant="body2" sx={{ color: palette.muted }}>
                  Approve delivery proof before invoice preparation. Click a
                  record to preview the file.
                </Typography>
              </Box>
              <Chip
                label={`${pendingPods.length} pending`}
                sx={{
                  ...
                  >
                    ...
                    >
                      <Box {...previewTriggerProps(pod)}>
                        ...
                        >
                          {driverName(pod.driverId)} ·{" "}
                          {new Date(
                            pod.uploadDate || Date.now(),
                          ).toLocaleDateString()}
                        </Typography>
                        <Typography
                          variant="body2"
                      ...
                      >
                        <Button
                          disabled={actionId === pod._id}
                          variant="contained"
                          startIcon={<CheckCircleOutlineIcon />}
                          onClick={() => handleApprove(pod._id)}
                          sx={{
                            borderRadius: 3,
                            bgcolor: palette.ink,
                            fontWeight: 900,
            ...
            {selectedDriver && (
              <>
                <TextField
                  ...
                  label="From"
                  InputLabelProps={{ shrink: true }}
                  value={podsFilterDateFrom}
                  onChange={(e) => setPodsFilterDateFrom(e.target.value)}
                />
                <TextField
                  fullWidth
                  size="small"
                  ...
                  label="To"
                  InputLabelProps={{ shrink: true }}
                  value={podsFilterDateTo}
                  onChange={(e) => setPodsFilterDateTo(e.target.value)}
                />
                <Button
                  size="small"
                  variant="outlined"
                  onClick={clearPodsFilters}
        ...
        {loading ? (
          ...
          >
            ...
            <Typography sx={{ color: palette.muted }}>
              PODs are currently stored by driver, not by job.
            </Typography>
          </Paper>
        ) : pods.length === 0 ? (
          <Paper
          ...
          >
            ...
            <Typography sx={{ color: palette.muted }}>
              This driver has not uploaded delivery proof yet.
            </Typography>
          </Paper>
        ) : (
          <Stack spacing={2}>
